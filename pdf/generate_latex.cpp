    ...
    while(pos < content.size()){
...

        {
            ...
            if(end != string::npos){
                ...
                if(isInDesc){
                    // Close the listingframe, insert the blockdesc, and reopen the listingframe
                    string formatted_desc = block_desc.empty() ? "\n" : "\n\\end{listingframe}\n\\blockdesc{" + block_desc + "}\n\\begin{listingframe}[]{}\n";
                    block_desc_desc.emplace_back(formatted_desc);
                    description += BLOCK_DESC_MARK;
                }
                else {
                    // Keep the original behavior for inline code blocks
                    string formatted_code = block_desc.empty() ? "\n" : "\n@\\blockdesc{" + block_desc + "}@\n";
                    block_descs.emplace_back(formatted_code);
                    processed_content += "\n" + BLOCK_DESC_MARK;
                }
                can_begin = false;
                isInBlckDesc = false;
    ...
    for(auto& [section_name, subsections] : sections) if(!subsections.empty()){
        ...
        for(auto [filename, subsection_name, file_hash] : subsections){
            ...
            if (!description.empty()) {
                tex += "\\vspace{-4pt}\n";
                string desc_tex = "\\begin{listingframe}[]{}\n" + description + "\n\\end{listingframe}\n";
                
                // Clean up any empty listingframes generated by the un-nesting split
                string empty1 = "\\begin{listingframe}[]{}\n\n\\end{listingframe}\n";
                string empty2 = "\\begin{listingframe}[]{}\n\\end{listingframe}\n";
                size_t pos;
                while((pos = desc_tex.find(empty1)) != string::npos) desc_tex.erase(pos, empty1.length());
                while((pos = desc_tex.find(empty2)) != string::npos) desc_tex.erase(pos, empty2.length());
